...
export function SpotifyContextProvider({ children }: { children: React.ReactNode }) {
  ...
  React.useEffect(() => {
    ...
    if (
      typeof process.env.NEXT_PUBLIC_SPOTIFY_CLIENT_ID !== 'string' ||
      typeof process.env.NEXT_PUBLIC_SPOTIFY_REDIRECT_URI !== 'string'
    ) {
...
function useSpotifyControls(
  ...
  const setPlaybackVolumeMutation = useMutation({
    mutationFn: async (volume: number) => {
      ...
      if (isLoggedIn) {
        await spotifyFetchWithRefresh([
          'https://api.spotify.com/v1/me/player/volume' + `?volume_percent=${adjustedVolume}`,
          {
            method: 'PUT',
          },
        ]);
